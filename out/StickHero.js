...
jimp_1.default.read('./img/sc0.png').then((img) => {
    let height, width;
    ...
    for (let y = 100; y < height; ++y) {
        let clor = jimp_1.default.intToRGBA(img.getPixelColour(x, y));
    ...
    for (let x = 5; x < width; ++x) {
        let clor = jimp_1.default.intToRGBA(img.getPixelColor(x, startY));
        if (isColorBlack(clor)) {
    ...
    for (let x = startXSnd + 1; x < width; ++x) {
        let clor = jimp_1.default.intToRGBA(img.getPixelColor(x, startY + 20));
        if (!isColorBlack(clor)) {
            console.log(`x: ${x} y: ${startY} color: `, clor);
            lastXSnd = x - 1;
